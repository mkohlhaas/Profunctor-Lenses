...
withGrate ∷ ∀ s t a b. AGrate s t a b → ((s → a) → b) → t
withGrate g = unwrap (g (Grating \f → f identity))

cloneGrate ∷ ∀ s t a b. AGrate s t a b → Grate s t a b
cloneGrate g = grate (withGrate g)

cotraversed ∷ ∀ f a b. Distributive f ⇒ Grate (f a) (f b) a b
cotraversed = grate \f → cotraverse f identity

zipWithOf ∷ ∀ s t a b. Optic Zipping s t a b → (a → a → b) → s → s → t
zipWithOf g f = unwrap (g (Zipping f))

zipFWithOf ∷ ∀ f s t a b. Optic (Costar f) s t a b → (f a → b) → (f s → t)
zipFWithOf g f = unwrap (g (Costar f))

collectOf ∷ ∀ f s t a b. Functor f ⇒ Optic (Costar f) s t a (f a) → (b → s) → f b → t
