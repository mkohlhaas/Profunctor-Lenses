...
type Lens s t a b = ∀ p. Strong p ⇒ Optic p s t a b
...
-- |
-- | ```purescript
-- | _2 ∷ ∀ s t a b. Lens (Tuple s a) (Tuple t b) a b
-- | ```
-- |
...
type Prism s t a b = ∀ p. Choice p ⇒ Optic p s t a b
...
type Iso s t a b = ∀ p. Profunctor p ⇒ Optic p s t a b
...
type Traversal s t a b = ∀ p. Wander p ⇒ Optic p s t a b
...
-- | An indexed lens.
type IndexedLens i s t a b = ∀ p. Strong p ⇒ IndexedOptic p i s t a b
...
type AffineTraversal s t a b = ∀ p. Strong p ⇒ Choice p ⇒ Optic p s t a b
...
type Grate s t a b = ∀ p. Closed p ⇒ Optic p s t a b
...
type Getter s t a b = ∀ r. Fold r s t a b
...
type IndexedTraversal i s t a b = ∀ p. Wander p ⇒ IndexedOptic p i s t a b
