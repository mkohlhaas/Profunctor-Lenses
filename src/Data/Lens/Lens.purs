...
withLens ∷ ∀ s t a b r. ALens s t a b → ((s → a) → (s → b → t) → r) → r
withLens l f = case l (Shop identity \_ b → b) of Shop x y → f x y

cloneLens ∷ ∀ s t a b. ALens s t a b → Lens s t a b
...
cloneIndexedLens ∷ ∀ i s t a b. AnIndexedLens i s t a b → IndexedLens i s t a b
...
-- | ```
lensStore ∷ ∀ s t a b. ALens s t a b → s → Tuple a (b → t)
