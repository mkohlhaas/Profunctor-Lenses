...
instance Eq i ⇒ Index (i → a) i a where
  ix i = lens (\f → f i) \f a j → if i == j then a else f j

instance Index (Maybe a) Unit a where
  ix _ = _Just

instance Index (Identity a) Unit a where
  ix _ = _Newtype

instance Index (Array a) Int a where
...
instance Index (NEA.NonEmptyArray a) Int a where
  ix n = affineTraversal set pre
    where
...
instance Index (L.List a) Int a where
  ix n = affineTraversal set pre
    where
...
instance Ord a ⇒ Index (S.Set a) a Unit where
...
instance Ord k ⇒ Index (M.Map k v) k v where
...
instance Index (FO.Object v) String v where
  ix k = affineTraversal set pre
    where
