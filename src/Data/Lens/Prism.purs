...
-- | not match.
prism ∷ ∀ s t a b. (b → t) → (s → Either t a) → Prism s t a b
...
-- |   _ → Nothing
-- | ```
prism' ∷ ∀ s a. (a → s) → (s → Maybe a) → Prism' s a
...
-- | ```
nearly ∷ ∀ a. a → (a → Boolean) → Prism' a Unit
...
only ∷ ∀ a. Eq a ⇒ a → Prism a a Unit Unit
only x = nearly x (_ == x)
...
-- | review solidFocus Color.white == Solid Color.white
-- | ```
review ∷ ∀ s t a b. Review s t a b → b → t
review = under Tagged

clonePrism ∷ ∀ s t a b. APrism s t a b → Prism s t a b
clonePrism l = withPrism l \x y p → prism x y p

withPrism ∷ ∀ s t a b r. APrism s t a b → ((b → t) → (s → Either t a) → r) → r
withPrism l f = case l (Market identity Right) of
  Market g h → f g h

matching ∷ ∀ s t a b. APrism s t a b → s → Either t a
matching l = withPrism l \_ f → f

--| Ask if `preview prism` would produce a `Just`.
is ∷ ∀ s t a b r. HeytingAlgebra r ⇒ APrism s t a b → s → r
is l = either (const ff) (const tt) <<< matching l

--| Ask if `preview prism` would produce a `Nothing`.
isn't ∷ ∀ s t a b r. HeytingAlgebra r ⇒ APrism s t a b → s → r
isn't l = not <<< is l
...
-- | ```
below ∷ ∀ f s a. Traversable f ⇒ APrism' s a → Prism' (f s) (f a)
below k =
  withPrism k $ \bt seta →
    prism (map bt) $ \s →
