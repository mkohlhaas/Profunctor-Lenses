...
view ∷ ∀ s t a b. AGetter s t a b → s → a
view l = unwrap (l (Forget identity))

-- | View the focus of a `Getter` and its index.
iview ∷ ∀ i s t a b. IndexedFold (Tuple i a) i s t a b → s → Tuple i a
iview l = unwrap (l (Indexed $ Forget identity))

-- | Synonym for `view`, flipped.
viewOn ∷ ∀ s t a b. s → AGetter s t a b → a
viewOn s l = view l s

-- | Convert a function into a getter.
to ∷ ∀ s t a b. (s → a) → Getter s t a b
to f p = Forget (unwrap p <<< f)

cloneGetter ∷ ∀ s t a b. AGetter s t a b → Getter s t a b
cloneGetter g = to (view g)

-- | Combine two getters.
takeBoth ∷ ∀ s t a b c d. AGetter s t a b → AGetter s t c d → Getter s t (Tuple a c) (Tuple b d)
takeBoth l r = to (view l &&& view r)

-- | View the focus of a `Getter` in the state of a monad.
use ∷ ∀ s t a b m. MonadState s m ⇒ Getter s t a b → m a
use p = gets (_ ^. p)
