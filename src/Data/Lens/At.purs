...
instance At (Identity a) Unit a where
  at _ = lens (Just <<< unwrap) (flip maybe Identity)

instance At (Maybe a) Unit a where
  at _ = lens identity \_ → identity

instance Ord v ⇒ At (S.Set v) v Unit where
...
instance Ord k ⇒ At (M.Map k v) k v where
  at k =
    lens (M.lookup k) \m →
      maybe' (\_ → M.delete k m) \v → M.insert k v m

instance At (FO.Object v) String v where
  at k =
    lens (FO.lookup k) \m →
      maybe' (\_ → FO.delete k m) \v → FO.insert k v m

sans ∷ ∀ m a b. At m a b ⇒ a → m → m
