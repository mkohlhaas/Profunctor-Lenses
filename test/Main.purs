...
foo ∷ ∀ a b r. Lens { foo ∷ a | r } { foo ∷ b | r } a b
foo = prop (Proxy ∷ Proxy "foo")

bar ∷ ∀ a b r. Lens { bar ∷ a | r } { bar ∷ b | r } a b
bar = prop (Proxy ∷ Proxy "bar")

barAndFoo ∷ ∀ a b r. Getter' { bar ∷ a, foo ∷ b | r } (Tuple a b)
barAndFoo = takeBoth bar foo

fooGetter ∷ ∀ x. Getter' { foo ∷ x } x
fooGetter = foo

barGetter ∷ ∀ x. Getter' { bar ∷ x } x
barGetter = bar
...
fooBarGetter ∷ ∀ x. Getter' { foo ∷ { bar ∷ x } } x
...
bars ∷ ∀ a b. Traversal (Foo a) (Foo b) a b
...
i_2 ∷ ∀ a b c. IndexedLens Int (Tuple a b) (Tuple a c) b c
i_2 = ilens (\(Tuple _ b) → Tuple 0 b) (\(Tuple a _) b → Tuple a b)

-- Grates
aGrateExample ∷ ∀ a b. Grate (Tuple a a) (Tuple b b) a b
aGrateExample = grate \f → Tuple (f fst) (f snd)

collectOfTest ∷ ∀ f a b. Functor f ⇒ (a → Tuple b b) → f a → Tuple (f b) (f b)
